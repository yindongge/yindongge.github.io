...
function commit(){
	...
	if($("#verifycode").val().length!=4 && $("#verifycode").val().length!=6){
		$("#showvcode").html("<p class='false' id='false'><span class='glyphicon glyphicon-remove-sign'></span> 验证码错误！</p>");
		return false;
	}
	$("#showvcode").children("#false").remove();
    ...
    $.ajax({  
        ...
        data: $('#form').serialize(),
        success: function(data) {  
           if(data.code != 200 && data.code==410){
        	   $("#mobEmail").html("<p class='false'><span class='glyphicon glyphicon-remove-sign'></span>"+data.desc+"</p>");
