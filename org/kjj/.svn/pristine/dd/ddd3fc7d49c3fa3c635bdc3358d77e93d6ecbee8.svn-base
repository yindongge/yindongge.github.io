...
import com.kjj.mobile.pay.alipay.util.AlipayNotify;
import com.kjj.mobile.pay.alipay.util.AlipaySubmit;
...
public class AlipayController {
	...
	public void pay(HttpSession session,HttpServletRequest request, HttpServletResponse response,PrintWriter out,Integer orderId) {
		...
		//是否检查支付宝app("Y"，尝试唤起支付宝客户端进行支付，若用户未安装支付宝，则继续使用wap收银台进行支付)
	...
	public void notifyUrl(HttpServletRequest request,PrintWriter out,Model model) {
		...
		if(AlipayNotify.verify(params)){//验证成功
			...
			if(trade_status.equals("TRADE_FINISHED")){
				...
				orgOrderService.updatePayOnline(Integer.parseInt(out_trade_no),OrgOrderConstant.ONLINE_PAY_STYLE_MOBILE_ALIPAY);
				//注意：
