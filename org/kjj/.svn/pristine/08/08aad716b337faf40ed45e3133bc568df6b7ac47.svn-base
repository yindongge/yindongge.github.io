...
public class MobilePageController {
	...
	public String customize(Model model,HttpServletRequest request, OrgMobilePageCustomizeQuery query){
		OrgMobilePage page = orgMobilePageService.queryById(query.getPageId());
		model.addAttribute("page", page);
		OrgMobilePageCustomize customize = orgMobilePageCustomizeService.queryOnlyOne(query);
		request.setAttribute("customize", customize != null ? customize.getHtmlText() : "");
		return "mobilePage/customize";
	}
	
	
	
	@ResponseBody
	@RequestMapping(value = "/saveCustomize", method = {RequestMethod.GET, RequestMethod.POST})
	public Map<String,Object> saveCustomize(Model model,OrgMobilePageCustomizeQuery query){
		Map<String,Object> map = new HashMap<String,Object>();
		String htmlText = query.getHtmlText();
		query.setHtmlText(null);
		OrgMobilePageCustomize customize = orgMobilePageCustomizeService.queryOnlyOne(query);
		if(null == customize && !"".equals(htmlText)){
			query.setHtmlText(htmlText);
			orgMobilePageCustomizeService.add(query);
			map.put("code", HttpStatusCode.CODE_SUCCESS);
			return map;
		}else if(null == customize && "".equals(htmlText)){
			map.put("code", HttpStatusCode.CODE_SUCCESS);
			return map;
		}else if(null != customize && "".equals(htmlText)){
			query.setId(customize.getId());
			orgMobilePageCustomizeService.delete(query);
			map.put("code", HttpStatusCode.CODE_SUCCESS);
			return map;
		}
		query.setHtmlText(htmlText);
		query.setId(customize.getId());
		orgMobilePageCustomizeService.updateByIdSelective(query);
		map.put("code", HttpStatusCode.CODE_SUCCESS);
		return map;
	}
	 ...
	 */
//	@RequestMapping(value = "/saveCustomize", method = {RequestMethod.GET, RequestMethod.POST})
//	public String saveCustomize(Model model,OrgMobilePageCustomizeQuery query){
//		String htmlText = query.getHtmlText();
//		query.setHtmlText(null);
//		OrgMobilePageCustomize customize = orgMobilePageCustomizeService.queryOnlyOne(query);
//		if(null == customize){
//			query.setHtmlText(htmlText);
//			orgMobilePageCustomizeService.add(query);
//			return "redirect:/mobilePage/customize?pageId="+query.getPageId();
//		}
//		query.setHtmlText(htmlText);
//		query.setId(customize.getId());
//		orgMobilePageCustomizeService.updateByIdSelective(query);
