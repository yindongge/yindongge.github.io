...
function(t) {
    var e = {
        ...
        _right: function() {
            ...
            e.right === !0 && this.value.slice(n, n + e.tab.length) == e.tab && (this.evt.preventDefault(), this.selectionStart = i + (this.evt.shiftKey ? 0 : e.tab.length), this.selectionEnd = n + e.tab.length)
        },
        _shiftTab: function() {
            this.evt.preventDefault();
            var e = t(this).data("settings"),
            ...
            if (preselection = this.value.slice(0, i), selection = this.value.slice(i, n), postselection = this.value.slice(n), isMultipleLine = selection.indexOf("\n") >= 0, lineStart = preselection.lastIndexOf("\n"), lineEnd = n + postselection.indexOf("\n"), isFirst = 0 == i, previousCharacter = isFirst ? "": this.value.slice(i - 1, i), 0 > lineStart ? lineStart = 0 : lineStart++, 0 > lineEnd && (lineEnd = this.value.length), isMultipleLine) {
                ...
                i - lineStart < e.tab.length && (u = i - lineStart, i == n && (takedEnd = u)),
...
$(function(){
	$('.all-sort-list > .item').hover(function(){
			...
			var	h = $('.all-sort-list').offset().top;

		  	var	s = $(window).scrollTop();

			var	i = $(this).offset().top;

			var	item = $(this).children('.item-list').height();

			var	sort = $('.all-sort-list').height();						//父类分类列表容器的高度
