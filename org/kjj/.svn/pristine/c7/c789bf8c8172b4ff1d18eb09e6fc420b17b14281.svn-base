	$(function () {
		...
		$("#sendAdd").click(function(){
			...
			//校验
	        $('#shopAdd').data('bootstrapValidator').updateStatus('sendAdd', 'NOT_VALIDATED', null).validateField('sendAdd');
		});
		...
		$(".close").on("click",function(){
			$(this).parent(".send_img_wrapper").find("img").attr("src","");
			$(this).parent(".send_img_wrapper").find("input:eq(1)").attr("value","");
		});
		
	});
	
	function uploadImage(imgId){
		var id = imgId.replace('_hid','');
	$.ajaxFileUpload({
		...
		success:function(data){
			if(data.code == 200){
				...
				$("#"+imgId).attr("value",data.returnImgUrl);
			}
		},
		error:function(data){}
	});
	...
	function showMap(mapArea) {
		...
		map.addEventListener("click", function(e) {
		    ...
		    map.addOverlay(circle);
		  	//校验
	        $('#shopAdd').data('bootstrapValidator').updateStatus('search', 'NOT_VALIDATED', null).validateField('search');
		});
		//搜索
