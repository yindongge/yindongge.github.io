...
<body>
	...
	<form id="pageform" action="${ctx }/advertisement/edit" method="post">
		<input type="hidden" name="advertisementId" id="advertisementId" value="${advertisementQuery.advertisementId }"/>
		<input type="hidden" name="listPageNum" value="${listPageNum }"/>
		<input type="hidden" name="classLevel1" value="${classLevel1 }"/>
		<input type="hidden" name="classLevel2" value="${classLevel2 }"/>
		<div class="row">
			<div class="col-md-6">
				<div class="form-horizontal">
					<div class="form-group">
						...
						<div class="col-md-8">
							<select class="form-control" name="classLevel1" id="classLevel1" disabled="disabled">
					...
					<div class="form-group">
						...
						<div class="col-md-8">
							<select class="form-control" name="classLevel2" id="classLevel2" disabled="disabled">
...
<!--分割线-->
		...
		<table class="table table-bordered table-hover borded-striped table-control">
			...
			<tbody>
				<c:forEach items="${advertisementItemList }" var="item">
				<tr>
					<td>${item.goodsSn}</td>
					<td>${item.goodsName}</td>
					<td>${item.productBrandName}</td>
					<td><img height="40" src="${item.goodsThumb}"></img> </td>
					<td>
						<c:if test="${ item.isOnSale==1}">
							上架
						</c:if>
						<c:if test="${ item.isOnSale==0}">
							下架
						</c:if>
					</td>
					<td>
						<button type="button" class="btn btn-danger btn-xs" onclick="cancel(${item.advertisementItemId})">取消推荐</button>
					</td>
				</tr>					
				</c:forEach>
			</tbody>
	...
	<div class="text-center">
			<button type="button" class="btn btn-primary " onclick="window.location.href='${ctx}/advertisement/list?pageNumber=${listPageNum }&classLevel1=${classLevel1 }&classLevel2=${classLevel2 }'">返回</button>
	</div>
</div>
<%@include file="../common/common_js.jsp" %>
<script src="${jsBase}/advertisement/edit.js" type="text/javascript"></script>
</body>
</html>