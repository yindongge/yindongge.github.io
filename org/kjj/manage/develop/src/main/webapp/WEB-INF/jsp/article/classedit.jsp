...
<div class="modal modal-control" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			...
			<form id="formAdd" name="formAdd" class="horizontal" method="post" action="${ctx}/articleClass/save">
						 ...
						 <select id="parentid" name="parentid" class="form-control">
						  <option value="-1">请选择</option>
							 <c:forEach items="${list}" var="i" >
							 	...
							 	<option  value="${i.id}" <c:if test="${i.id eq obj.parentid}">selected </c:if>><c:if test="${i.parentid ne -1}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</c:if>${i.className}</option>
							 </c:forEach>
						 </select>
					...
					<div class="form-group">
						 ...
						 <input type="text" class="form-control" id="order" name="order" value="${obj.order}" placeholder="255"/>		
					</div>
