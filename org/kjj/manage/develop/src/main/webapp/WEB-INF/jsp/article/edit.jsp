...
<form action="${ctx}/article/save" method="post" id="form" name="form" enctype="multipart/form-data">
	<input type="hidden" id="shops" name="shops" value="${shopsid}"></input>
	<input type="hidden" id="articleId" name="articleId" value="${obj.id}"></input>
	<ul class="breadcrumb">
	...
	<table class="table table-hover table-bordered ">
		<tbody>
			<tr>
				...
				<td class="c-length2">
					<input type="text" id="title" name="title" value="${obj.title}" placeholder="2-10 长度字符！" />
				</td>		
			</tr>
			<tr>
				<td class="c-length1">文章副标题：</td>
				<td class="c-length2">
					<input type="text" id="subtitle" name="subtitle" value="${obj.subtitle}" placeholder="支付流程" />
				</td>		
			</tr>
			<tr>
				...
				<td class="c-length2">
							...
							<div id="div_1">
										<img  id="showimg_1" name="showimg_1" src="${obj.image}" height="30"></img>
							</div>		
			...
			<tr>
					...
					<select  id="articleClass" name="classid">
						<c:forEach items="${classList}" var ="class0" varStatus="status0">
							<c:if test="${! empty class0}">      	
	      						<option value="${class0.id}" <c:if test="${obj.classid==class0.id }"> selected </c:if>>${status0.index+1} ${class0.className}</option>
	      						<c:forEach items="${class0.listSubClass}" var ="class1" varStatus="status1">
		      						<c:if test="${! empty class1}">      	
		      							<option value="${class1.id}" <c:if test="${obj.classid==class1.id }"> selected </c:if>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${status0.index+1} .${status1.index+1} ${class1.className}</option>
		      							 <c:forEach items="${class1.listSubClass}" var ="class2" varStatus="status2">
				      						<c:if test="${! empty class2}">      	
				      							<option value="${class2.id}" <c:if test="${obj.classid==class2.id }"> selected </c:if>>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${status0.index+1} .${status1.index+1} .${status2.index+1} ${class2.className}</option>
				      						</c:if>
			      						</c:forEach>
		      						</c:if>
	      						</c:forEach>
				...
				<span style="float:left;padding-left:30px;line-height:30px;display:block" id="show">${shopsname}</span>
			...
			<tr>
				...
				<td class="c-length2">
					<input type="text" id="url" name="url" value="${obj.url}" placeholder="http" />
			</tr>
			<tr>
				<td class="c-length1">优先级：</td>
				<td class="c-length2">
					<input type="text" id="order" name="order" value="${obj.privelege}" placeholder="请填写1-255正整数" />
				</td>		
			</tr>
		</tbody>
	</table>
	<div class="panel ">
		<div class="panel-body" style="padding-left:0px;">
			<textarea style="width:80%" rows="10" id="content" name="content">${obj.content}		
			</textarea>
...
<script src="${jsBase}/article/edit.js" type="text/javascript"></script>
</body>