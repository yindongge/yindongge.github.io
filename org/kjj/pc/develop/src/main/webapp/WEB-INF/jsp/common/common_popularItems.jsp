...
$(function(){
			    	...
			    	if(data.page.content==undefined){
				    	...
				    	$.each(data.page.content,function(index,obj){
			    		    ...
			    		    if(obj.orgProductItemAide.canSale){
								divHtml+="<P><button data-goodsId=\""+obj.orgProductItem.goodsId+"\" data-goodsSn=\""+obj.orgProductItem.goodsSn+"\"  type=\"button\" class=\"btn btn-warning btn-sm\">立即抢购</button></P>";
			    		    }else{
	...
	$('#popularItems').on('click',':button',function(e){
		addCart($(this).attr("data-goodsId"),$(this).attr("data-goodsSn")+"",1);
		animateAddCart($('.side_item.side_2'),$(this).parents('li').find('img'));
		e.preventDefault();
	});
});
