...
public class AccountDepositController {
	...
	public String findPasswordInit(Model model,HttpSession session) {
		OrgUsersSession user = (OrgUsersSession) session.getAttribute(SessionConstant.SESSION_USER);
		if(StringUtils.isNoneBlank(user.getOrgUsers().getMobilePhone()) || StringUtils.isNoneBlank(user.getOrgUsers().getEmail())){
	...
	@ResponseBody
	@RequestMapping(value = "/findAccountPassword" ,method = { RequestMethod.GET,RequestMethod.POST })
	public Map<String,Object> findAccountPassword(HttpSession session,OrgAccountDeposit orgAccountDeposit,String verifycode,String mobileOrEmail){
		//获取用户
		OrgUsersSession user = (OrgUsersSession) session.getAttribute(SessionConstant.SESSION_USER);
		...
		if(mobileOrEmail.contains("@")){
			...
			if(!mobileOrEmail.equals(user.getOrgUsers().getEmail())){
				final int  CODE_ERROR_NOUSER=410;
				mapResult.put("code",CODE_ERROR_NOUSER);
				mapResult.put("desc", "请输入已绑定的手机号或邮箱");
				return mapResult;
			}
		}else{
			query.setPhone(mobileOrEmail);
			if(!mobileOrEmail.equals(user.getOrgUsers().getMobilePhone())){
			final int  CODE_ERROR_NOUSER=410;
			mapResult.put("code",CODE_ERROR_NOUSER);
			mapResult.put("desc", "请输入已绑定的手机号或邮箱");
			return mapResult;
		...
		if(StringUtils.isBlank(verifycode) || verifycode.length() != 4 && verifycode.length() != 6){
