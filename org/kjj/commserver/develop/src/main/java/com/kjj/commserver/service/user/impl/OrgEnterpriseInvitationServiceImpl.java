...
public class OrgEnterpriseInvitationServiceImpl extends BaseServiceImpl<OrgEnterpriseInvitation, Integer> implements OrgEnterpriseInvitationService {
    ...
    @Resource
    private OrgEnterpriseInvitationUserService orgEnterpriseInvitationUserService;
	...
	public boolean updateInvitation(String invitationCode, OrgUsers users) {
		...
		if(invitationCode.length() != 6 && invitationCode.length() != 9) {
			return false;
		}
		
		if(invitationCode.length() == 6) {
			OrgEnterpriseEasyInvitation easyInvitation = orgEnterpriseEasyInvitationService.lockQueryByInvitationCode(invitationCode);
			if(null == easyInvitation){
				return false;
			}
			// 判断邀请码是超过有效时间
			Date today = new Date();
			if(today.getTime() >= easyInvitation.getStartTime().getTime() && today.getTime() <= easyInvitation.getEndTime().getTime()){
				// 剩余激活数是否为零
				int rest = easyInvitation.getRest(); 
				if(rest > 0){
					// 将剩余激活数减一
					easyInvitation.setRest(rest - 1);
					orgEnterpriseEasyInvitationService.updateById(easyInvitation);
					
					// 创建用户使用邀请码记录信息
					OrgEnterpriseInvitationUser easyInvitationUser = new OrgEnterpriseInvitationUser();
					easyInvitationUser.setCreateTime(today);
					easyInvitationUser.setEasyInvitationId(easyInvitation.getId());
					easyInvitationUser.setUserId(users.getUserId());
					orgEnterpriseInvitationUserService.add(easyInvitationUser);
					
					// 创建企业与用户关联
					OrgEnterpriseUser eu = new OrgEnterpriseUser();
					eu.setEnterpriseId(easyInvitation.getEnterpriseId());
