...
<div class="box box-gray">
		<header class="header">
			<div class="head-content">订单详情</div>
	...
	<div class="dingdan-wrapper">
		<div class="list-detail">
			...
			<c:choose>
			    <c:when test="${order.status==0 && order.payStatus==0}">
				    <div class="hidea">
						<a href="javascript:cancelOrder(${order.orderId});" class="bg-yellow">申请取消订单</a>
						<c:if test="${!empty kjjopenid}">
							<a href="javascript:void(0);" class="bg-yellow" onclick="wxpay(${order.orderId});">去支付</a>
						</c:if>
						<c:if test="${empty kjjopenid}">
							<a href="${ctx}/alipay/pay?orderId=${order.orderId}" class="bg-yellow">去支付</a>
						</c:if>
					</div>
			    </c:when>
			    <c:when test="${order.status==1 || order.status==2}">
					<a href="javascript:cancelOrder(${order.orderId});" class="btnsure">申请取消订单</a>
			    </c:when>
			</c:choose>
		</div>
		<div class="list-detail ">
