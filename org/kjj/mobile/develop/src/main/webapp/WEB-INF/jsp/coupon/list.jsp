				...
				<div class="youhuilist" data-coupon-id="${item.couponId}">
					<div class="youhui_top <c:if test="${!query.statusCanUse}">gray</c:if>">
						<div class="youhuicontent">
							...
							<p>
								<c:if test="${item.orgCoupon.productType==1}">全场通用</c:if>
								<c:if test="${item.orgCoupon.productType==2}"><c:forEach var="className" items="${item.orgCoupon.classNames}">${className},</c:forEach></c:if>
